extends RefCounted
class_name AllyCoreMemory
# ─────────────────────────────────────────────────────────────────────────────
# AllyCoreMemory -- port of tribe's npc_core_memory.gd (2026-08-26 parity audit).
# Same SSH edge-vs-bulk topological-protection mechanism, retargeted from
# tribe's social NPCs to horde-beta's team allies:
#
#   CORE memories (edge slots, 1-2 hops from a chain boundary) survive combat
#   panic; routine memories (bulk slots, ~5-6 hops) wash out under the same
#   panic. So an ally that was nearly killed remembers "I almost died here"
#   through a chaotic firefight, while petty scrapes fade -- memory salience
#   emerges from chain topology instead of an arbitrary flag.
#
# Depends only on res://spikeling.gd (identical API in both repos:
# load_from_text / stimulate_idx / step / synapses[].src/.dst/.weight).
# See tribe repo npc_core_memory.gd header for the confirmed experiment.
# ─────────────────────────────────────────────────────────────────────────────
...
## ones. Callers combine this with recall() to decide whether a memory is
...
func _build_chain() -> void:
	var lines: Array[String] = ["# Ally core-memory SSH chain"]
	for i in range(N):
